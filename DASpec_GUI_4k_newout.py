...
font_text = ("Arial", 9)
#font_text = ("Times New Roman", 10, "bold")

win_geometry = {"main": "640x550+350+0", 
                "author": "285x200+350+350", 
		"model": "820x600+1050+0", 
		"result": "640x400+1050+700", 
		"fit": "350x60+0+370", 
		"tie": "650x300+0+820",
		"fix": "300x300+0+470",
		"win": "300x300+0+0"}
...
class GUI(Tkinter.Frame):
    ...
    def __init__(self):
        ...
        self.parent.geometry(win_geometry["main"])
        self.initUI()
    ...
    def initUI(self):
        ...
        fileMenu.add_command(label = "single", command = self.single_spectrum, font = font_text)
        fileMenu.add_command(label = "batch", command = self.batch_spectra, font = font_text)
        fileMenu.add_command(label = "Exit", command = self.parent.quit, font = font_text)
        menubar.add_cascade(label = "Spectrum", menu = fileMenu, font = font_text)

        fileMenu = Tkinter.Menu(menubar)
        fileMenu.add_command(label = "Load", command = self.set_model_from_text, font = font_text)
        menubar.add_cascade(label = "Set model", menu = fileMenu, font = font_text)

        fileMenu = Tkinter.Menu(menubar)
        fileMenu.add_command(label = "About DASpec", command = self.authorinfo, font = font_text)
        menubar.add_cascade(label = "Info", menu = fileMenu, font = font_text)

    def authorinfo(self):
        t = Tkinter.Toplevel(self.parent)
        t.title("About")
        t.geometry(win_geometry["author"])
        l = Tkinter.Label(t, text = "Decomposition of AGN Spectrum (DASpec)", font = font_text)
        l.grid(row = 0, column = 0)
        l = Tkinter.Label(t, text = "Version 0.9", font = font_text)
        l.grid(row = 1, column = 0)
        l = Tkinter.Label(t, text = "", font = font_text)
        l.grid(row = 2, column = 0)
        l = Tkinter.Label(t, text = "Pu Du", font = font_text)
        l.grid(row = 3, column = 0)
        l = Tkinter.Label(t, text = "Institute of High Energy Physics", font = font_text)
        l.grid(row = 4, column = 0)
        l = Tkinter.Label(t, text = "Chinese Academy of Sciences", font = font_text)
        l.grid(row = 5, column = 0)
        l = Tkinter.Label(t, text = "", font = font_text)
        l.grid(row = 6, column = 0)
        l = Tkinter.Label(t, text = "July 2025", font = font_text)
        l.grid(row = 7, column = 0)
        l = Tkinter.Label(t, text = "All rights reserved.", font = font_text)
    ...
    def addmodel(self):
        ...
        for j in range(len(self.addmodel_info)):
        ...
        for iwin in [len(self.models) - 1]:
            ...
            for ipar in range(len(self.models[iwin]) - 3):
                p = Tkinter.Entry(self.windows[1], width = 8, font = font_text)
                self.model_info[iwin].append(p)
                p.grid(row = iwin1, column = col)
                p.insert(0, str(self.models[iwin][col - 1]))
                col += 1
            self.model_info[iwin].append([])
            for ipar in range(len(self.models[iwin][-2])):
                p = Tkinter.Entry(self.windows[1], width = 8, font = font_text)
    ...
    def addmodel_from_known(self, modelname, modelpar, par, limits):
        ...
        for j in range(len(self.models[-1][-2])):
            self.models[-1][-1][j * 2] = limits[j * 2]
            self.models[-1][-1][j * 2 + 1] = limits[j * 2 + 1]
    ...
    def set_model_from_file(self, modelfile):
        ...
        limits = [i for i in reduced_l if "limits of comp" in i]
        ...
        for i in range(len(models)):
            ...
            limit = [float(j) for j in limits[i].split(":")[1].split()]
            print(modelname, modelpar, par)
            self.addmodel_from_known(modelname, modelpar, par, limit)
        self.selectallmodel()
        ...
        if len(ties) != 0:
            for i in range(len(ties)):
                ...
                if len(temp.split()) == 8:
                    ...
                    tiepar = []
                    for j in range(len(temp.split()) - 1):
                        #print(j)
                    ...
                    tiepar = []
                    for j in range(len(temp.split()) - 2):
        ...
        if len(fixes) != 0:
            for i in range(len(fixes)):
                fixpar = []
                for j in range(len(fixes[i].split()) - 1):
    ...
    def set_model(self):
        ...
        t = Tkinter.Toplevel(self.parent)
        t.title("model")
        t.geometry(win_geometry["model"])

        scrolled_frame = ScrolledFrame(t)
        scrolled_frame.pack(fill="both", expand=True)
        self.windows.append(scrolled_frame.inner_frame)
        inner_frame = scrolled_frame.inner_frame

        Tkinter.Button(self.windows[1], text = "All", command = self.selectallmodel, font = font_text).grid(row = 0, column = 0, sticky = Tkinter.W)
        Tkinter.Button(self.windows[1], text = "None", command = self.deselectallmodel, font = font_text).grid(row = 0, column = 1, sticky = Tkinter.W)
        m = Tkinter.Menubutton(self.windows[1], text = "Add", font = font_text)
        ...
        for i in DASpec.model_name_p:
            ...
            m.menu.add_checkbutton(label = i, variable = var, command = self.addmodel, font = font_text)
        Tkinter.Button(self.windows[1], text = "Delete", font = font_text, command = self.delmodel).grid(row = 0, column = 3, sticky = Tkinter.W)
        Tkinter.Button(self.windows[1], text = "Use previous result", font = font_text, command = self.model_useresult).grid(row = 0, column = 12, sticky = Tkinter.W)


        iwin1 = 1
        for iwin in range(len(self.models)):
            self.model_info.append([Tkinter.IntVar()])
            c1 = Tkinter.Checkbutton(self.windows[1], text = str(iwin + 1), font = font_text,
                variable = self.model_info[iwin][0])
            self.model_info[iwin].append(c1)
            c1.grid(row = iwin1, column = 0, sticky = Tkinter.W)
            m = Tkinter.Menubutton(self.windows[1], text = self.models[iwin][0], font = font_text, width = 22)
            self.model_info[iwin].append(m)
            m.grid(row = iwin1, column = 1)
            m.menu = Tkinter.Menu(m)
            m["menu"] = m.menu
    ...
    def updatemodel(self):
        for i in range(len(self.model_info)):
            #print(self.model_info)
            for j in range(len(self.model_info[i][3])):
                #print(self.model_info[i][-1][j].get())
    ...
    def set_resultwin(self):
        t = Tkinter.Toplevel(self.parent)
        t.title("result")
        t.geometry(win_geometry["result"])
        ...
        text = Tkinter.Text(inner_frame, width = 180, height = 60)
        self.resultwin = text
    ...
    def set_fitwin(self):
        ...
        self.windows[4].geometry(win_geometry["fit"])
        self.windows[4].title("fit")
        #button = Tkinter.Button(master=self.windows[4], text='set fit', command=self.set_fit)
        #button.grid(row = 0, column = 0)
        button = Tkinter.Button(master=self.windows[4], text='lmfit', font = font_text, command=self.lmfit)
        button.grid(row = 0, column = 0)
        button = Tkinter.Button(master=self.windows[4], text='mix_fit', font = font_text, command=self.mixfit)
        button.grid(row = 0, column = 1)
        button = Tkinter.Button(master=self.windows[4], text='siman', font = font_text, command=self.siman)
        button.grid(row = 0, column = 2)
        button = Tkinter.Button(master=self.windows[4], text='next', font = font_text, command=self.nextspec)
        button.grid(row = 0, column = 3)
        button = Tkinter.Button(master=self.windows[4], text='nextlmfit', font = font_text, command=self.next_lmfit)
        button.grid(row = 1, column = 0)
        button = Tkinter.Button(master=self.windows[4], text='autolmfit', font = font_text, command=self.auto_lmfit)
        button.grid(row = 1, column = 1)
        button = Tkinter.Button(master=self.windows[4], text='nextmixfit', font = font_text, command=self.next_mixfit)
        button.grid(row = 1, column = 2)
        button = Tkinter.Button(master=self.windows[4], text='automixfit', font = font_text, command=self.auto_mixfit)
    ...
    def set_tie(self):
        ...
        Tkinter.Button(self.windows[3], text = "Update", font = font_text, command = self.updatetie).grid(row = 0, column = 0, sticky = Tkinter.W)

        Tkinter.Button(self.windows[3], text = "All", font = font_text, command = self.selectalltie).grid(row = 1, column = 0, sticky = Tkinter.W)
        Tkinter.Button(self.windows[3], text = "None", font = font_text, command = self.deselectalltie).grid(row = 1, column = 1, sticky = Tkinter.W)
        #Tkinter.Button(self.windows[3], text = "Add", command = self.addtie).grid(row = 1, column = 2, sticky = Tkinter.W)
        m = Tkinter.Menubutton(self.windows[3], text = "Add", font = font_text)
        ...
        for i in DASpec.tie_name:
            ...
            m.menu.add_checkbutton(label = i, variable = var, command = self.addtie, font = font_text)
        Tkinter.Button(self.windows[3], text = "Delete", font = font_text, command = self.deltie).grid(row = 1, column = 3, sticky = Tkinter.W)
    ...
    def updatetie(self):
        ...
        self.fix = []
        for i in range(len(self.fix_info)):
        ...
        self.tie = []
        for i in range(len(self.tie_info)):
            if self.tie_info[i][0].get():
                if len(self.tie_info[i]) - 3 == 6:
                    ...
                    temp = []
                    for j in range(len(self.model_info[int(self.tie_info[i][4].get()) - 1][-3]) - 1):
                        temp.append(self.model_info[int(self.tie_info[i][4].get()) - 1][-3][j + 1].get())
                    for j in range(len(self.model_info[int(self.tie_info[i][3].get()) - 1][-3]) - 1):
    ...
    def addtie_from_known(self, tietype, tiepar):
        # for j in range(len(self.addtie_info)):
        ...
        if typ == 0:
            ...
            c1 = Tkinter.Checkbutton(self.windows[3], text = str(len(self.tie_info)), font = font_text
            ...
            l = Tkinter.Label(self.windows[3], text = "tie", font = font_text)
            self.tie_info[-1].append(l)
            l.grid(row = len(self.tie_info) + 1, column = 1, sticky = Tkinter.W)
            e11 = Tkinter.Entry(self.windows[3], width = 8, font = font_text)
            ...
            c1.grid(row = len(self.tie_info) + 1, column = 0, sticky = Tkinter.W)
            self.tie_info[-1].append(c1)
            l = Tkinter.Label(self.windows[3], text = "profile", font = font_text)
            self.tie_info[-1].append(l)
            l.grid(row = len(self.tie_info) + 1, column = 1, sticky = Tkinter.W)
            e11 = Tkinter.Entry(self.windows[3], width = 8, font = font_text)
            self.tie_info[-1].append(e11)
            ...
            c1.grid(row = len(self.tie_info) + 1, column = 0, sticky = Tkinter.W)
            self.tie_info[-1].append(c1)
            l = Tkinter.Label(self.windows[3], text = "flux_profile", font = font_text)
            self.tie_info[-1].append(l)
            l.grid(row = len(self.tie_info) + 1, column = 1, sticky = Tkinter.W)
            e11 = Tkinter.Entry(self.windows[3], width = 8, font = font_text)
            self.tie_info[-1].append(e11)
    ...
    def deltie(self):
        if self.tie_info != []:
            for i in range(len(self.tie_info[-1]) - 1):
    ...
    def set_fix(self):
        ...
        Tkinter.Button(self.windows[2], text = "Update", font = font_text, command = self.updatefix).grid(row = 0, column = 0, sticky = Tkinter.W)

        Tkinter.Button(self.windows[2], text = "All", font = font_text, command = self.selectallfix).grid(row = 1, column = 0, sticky = Tkinter.W)
        Tkinter.Button(self.windows[2], text = "None", font = font_text, command = self.deselectallfix).grid(row = 1, column = 1, sticky = Tkinter.W)
        Tkinter.Button(self.windows[2], text = "Add", font = font_text, command = self.addfix).grid(row = 1, column = 2, sticky = Tkinter.W)
        Tkinter.Button(self.windows[2], text = "Delete", font = font_text, command = self.delfix).grid(row = 1, column = 3, sticky = Tkinter.W)
    ...
    def addfix(self):
        ...
        c1 = Tkinter.Checkbutton(self.windows[2], text = str(len(self.fix_info)), font = font_text
        ...
        e11 = Tkinter.Entry(self.windows[2], width = 8, font = font_text)
    ...
    def set_fitwindow(self):
        ...
        Tkinter.Button(self.windows[0], text = "Update", font = font_text, command = self.updatewin).grid(row = 0, column = 0, sticky = Tkinter.W)
        Tkinter.Button(self.windows[0], text = "All", font = font_text, command = self.selectallwin).grid(row = 1, column = 0, sticky = Tkinter.W)
        Tkinter.Button(self.windows[0], text = "None", font = font_text, command = self.deselectallwin).grid(row = 1, column = 1, sticky = Tkinter.W)
        Tkinter.Button(self.windows[0], text = "Add", font = font_text, command = self.addwin).grid(row = 1, column = 2, sticky = Tkinter.W)
        Tkinter.Button(self.windows[0], text = "Delete", font = font_text, command = self.delwin).grid(row = 1, column = 3, sticky = Tkinter.W)

        for iwin in range(len(DASpec.fitwindows)):
            self.fitwindow_info.append([Tkinter.IntVar()])
            c1 = Tkinter.Checkbutton(self.windows[0], text = str(iwin + 1), font = font_text
                , variable = self.fitwindow_info[iwin][0])
            self.fitwindow_info[iwin].append(c1)
            c1.grid(row = iwin + 2, column = 0, sticky = Tkinter.W)
            e11 = Tkinter.Entry(self.windows[0], width = 8, font = font_text)
            self.fitwindow_info[iwin].append(e11)
            e11.insert(0, str(DASpec.fitwindows[iwin][0]))
            e11.grid(row = iwin + 2, column = 1)
            l = Tkinter.Label(self.windows[0], text = "-", font = font_text)
            self.fitwindow_info[iwin].append(l)
            l.grid(row = iwin + 2, column = 2)
            e12 = Tkinter.Entry(self.windows[0], width = 8, font = font_text)
    ...
    def addwin(self):
        ...
        self.fitwindow_info.append([Tkinter.IntVar()])
        c1 = Tkinter.Checkbutton(self.windows[0], text = str(iwin + 1), font = font_text, variable = self.fitwindow_info[iwin][0])
        self.fitwindow_info[iwin].append(c1)
        c1.grid(row = iwin + 2, column = 0, sticky = Tkinter.W)
        e11 = Tkinter.Entry(self.windows[0], width = 8, font = font_text)
        self.fitwindow_info[iwin].append(e11)
    ...
    def plotspec(self):
        ...
        if fl != '':
            ...
            mpl.rcParams['font.size'] = matplotlib_font_size
            fig = Figure(figsize = (8, 6), dpi = matplotlib_dpi)
            self.figspec = fig
    ...
    def lmfit(self):
        ...
        #print('test6')
        npar = []
        for i in range(len(self.curvefit.model.comps)):
        ...
        limits = self.curvefit.limits
        lowers = self.model.pars2l([i[1] for i in limits[0::2]])
        highers = self.model.pars2l([i[1] for i in limits[1::2]])
        limits_tot = []
        for i in range(len(lowers)):
            limits_tot.append([i, lowers[i], 0])
            limits_tot.append([i, highers[i], 1])

        u = 0
        for i in range(len(npar)):
            text = 'limits of comp %i: ' % (i + 1)
            for j in range(npar[i]):
                text += '%e %e ' % (limits_tot[2 * u][1], limits_tot[2 * u + 1][1])
                u += 1
            text += '\n'
            self.outputfile.write(text)
        ...
        for i in range(len(self.model_info)):
            if self.model_info[i][0].get():
                if self.models[i][0] in ["template_spec_gaussian", "template_spec_dgaussian", "template_spec_lorentzian"\
                    ...
                    for j in range(len(self.model_info[i]) - 7):
                        if j == 1:
                            text += "'%s'," % (profile_temp)
